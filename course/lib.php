...
function userbased_courselist($userid,$roleid){
    ...
    if($DB->record_exists('course_type',array('creatorid'=>$userid))){
        $content .='<div class="tab-content ">
                            <div class="tab-pane active" id="1">';
                            if($DB->record_exists('course_type',array('creatorid'=>$userid,'subscribed'=>0))){
                                $academic_courses = $DB->get_records('course_type',array('creatorid'=>$userid,'subscribed'=>0));;
                                $content .=courselist_tables($roleid,$academic_courses);
                            }else{
                                $content .= 'You do not  have any academic courses'; 
                            }
                            $content .='</div>
                            <div class="tab-pane" id="2">';
                            if($DB->record_exists('course_type',array('creatorid'=>$userid,'subscribed'=>1))){
                                $subscribed_courses = $DB->get_records('course_type',array('creatorid'=>$userid,'subscribed'=>1));
                                $content .=courselist_tables($roleid,$subscribed_courses);
                            }else{
                                $content .= 'You do not  have any subscribed courses'; 
                            }
                            $content .= '</div>
                            <div class="tab-pane" id="3">';
                            if($DB->record_exists('course_type',array('creatorid'=>$userid,'subscribed'=>2))){
                                $academic_subscribed_courses = $DB->get_records('course_type',array('creatorid'=>$userid,'subscribed'=>2));
                                $content .=courselist_tables($roleid,$academic_subscribed_courses);
                            }else{
                                $content .= 'You do not have any industry partner courses'; 
                            }
                            $content .= '</div>
                        </div>';
    }else{
        $content .= 'You have not created any courses';
    }
    $content .= '</div>';
...
function courselist_tables($role,array $courses){
    global $CFG,$DB;
    $crslist = '';
    $i = 1;
    $crslist .= '<table id="example1" class="table table-striped table-bordered datatable" style="width:100%">
                        <thead>
                            <tr>
                                <th>S.No</th>
                                <th>Course Code</th>
    ...
    $crslist .= '<tbody>';
                        foreach ($courses as $course) {
                            ...
                            $criteria = html_writer::link(new moodle_url('//#',array()),'Add Criteria');
                            $editcourse = html_writer::link(new moodle_url('/course/edit.php',array('id' => $crs->id)),'Edit',array('class'=>'courselistlink'));
                            $crslist .= '<tr>
                                            <td>'.$i.'</td>
                                            <td>'.$crs->shortname.'</td>
                                            <td>'.$crs->fullname.'</td>
                                            <td>'.$crs_cat->name.'</td>';
                            $crslist .= '<td><button type="button" class="btn btn-info crslink">'.$criteria.'</button></td>
                                         <td>'.$editcourse.'</td>';
                            $crslist .='</tr>';
                            $i++;
                        }  
