...
if (! $batch_instance = $DB->get_record("batch", array("id"=>$data->batchid))) {
    message_paypal_error_to_admin("Not a valid batch id", $data);
    die;
}
...
/*nihar
        ...
        if ($data->payment_status == "Pending" and $data->pending_reason != "echeck") {
            ...
            $eventdata->name              = 'Batch_payment';
            $eventdata->userfrom          = get_admin();
            $eventdata->userto            = $user;
            $eventdata->subject           = "Moodle: PayPal payment for batch";
            $eventdata->fullmessage       = "Your PayPal payment for batch is pending.";
        ...
        if ($existing = $DB->get_record("enrol_batch", array("txn_id"=>$data->txn_id))) {   // Make sure this transaction doesn't exist already
        ...
        if (!$course = $DB->get_record('course', array('id'=>$data->courseid))) { // Check that course exists
            message_paypal_error_to_admin("Course $data->batchid doesn't exist", $data);
            die;
        }
        ...
        // ALL CLEAR !
         $batch = $DB->get_record('batch', array('id' => $data->batchid));
        $data->option_name2 = $batch->discountcost;
        $DB->insert_record("enrol_batch", $data);

        // Enrol user commented by moodleofindia
       // $plugin->enrol_user($plugin_instance, $user->id, $plugin_instance->roleid, $timestart, $timeend);
        ...
        if (!empty($mailstudents)) {
            $a = new stdClass();
             $site = get_site();
             $a->batch = $batchname;
	     $a->startdate = userdate($batch->startdate, get_string('strftimedate'));
	     $a->enddate   = userdate($batch->enddate, get_string('strftimedate'));
             $a->discountcost = get_config('enrol_paypal','currency').' '.$batch->discountcost;
	     $a->actualcost   = get_config('enrol_paypal','currency').' '.$batch->actualcost;
	     $a->duration = $batch->duration.' '.get_string('hrs','local_course_batches');
	     $a->days   = $batch->days;
             
             $a->batchurl = "$CFG->wwwroot/my/coursedetails.php?id=$course->id";
            ...
            $eventdata->userto            = $user;
            $eventdata->subject           = get_string("enrolmentnew", 'local_course_batches', $batchname);
            $eventdata->fullmessage       = get_string('welcometocoursetext', 'local_course_batches', $a);
            $eventdata->fullmessageformat = FORMAT_PLAIN;
            $eventdata->fullmessagehtml   = '';
            $eventdata->smallmessage      = '';
            message_send($eventdata);
        ...
        if (!empty($mailteachers) && !empty($teacher)) {
            ...
            $eventdata->subject           = get_string("enrolmentnew", 'local_course_batches', $shortname);
	    ...
	    $a->batch = $batchname;
	     ...
	     $a->days   = $batch->days;
            ...
            foreach ($admins as $admin) {
                ...
                $eventdata->subject           = get_string("enrolmentnew",'local_course_batches', $batchname);
                $eventdata->fullmessage       = get_string('enrolmentnewuser','local_course_batches', $a);
                $eventdata->fullmessageformat = FORMAT_PLAIN;
                $eventdata->fullmessagehtml   = '';
                $eventdata->smallmessage      = '';
                message_send($eventdata);
