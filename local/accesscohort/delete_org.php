...
$PAGE->set_url('/local/accesscohort/delete.php');
echo $OUTPUT->header();
if(is_siteadmin()){
	$delete = $DB->delete_records('local_organization', array('id' => $id));
	if($delete) {
		redirect(new moodle_url($CFG->wwwroot.'/local/accesscohort/list_org.php'),'Delete');
	} else {
		echo "error";
	}
}else{
