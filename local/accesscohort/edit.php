...
require($CFG->dirroot.'/local/accesscohort/form/edit_form.php');
...
//we need to change capability!!!
require_capability('moodle/cohort:manage', $context);

if ($returnurl) {
    $returnurl = new moodle_url($returnurl);
} else {
    $returnurl = new moodle_url('/local/accesscohort/index.php', array('contextid'=>$context->id));
}
...
$baseurl = new moodle_url('/local/accesscohort/edit.php', array('contextid' => $context->id, 'id' => $cohort->id));
...
if ($delete and $cohort->id) {
    ...
    if ($confirm and confirm_sesskey()) {
        aceesscohort_delete_cohort($cohort);
        redirect($returnurl);
    }
    ...
    $yesurl = new moodle_url('/local/accesscohort/edit.php', array('id' => $cohort->id, 'delete' => 1,
...
if ($show && $cohort->id && confirm_sesskey()) {
    if (!$cohort->visible) {
        ...
        aceesscohort_update_cohort($record);
    }
    redirect($returnurl);
}
...
$editform = new aceesscohort_edit_form(null, array('editoroptions'=>$editoroptions, 'data'=>$cohort, 'returnurl'=>$returnurl));
...
if ($editform->is_cancelled()) {
    ...
    if ($data->id) {
        ...
        aceesscohort_update_cohort($data);
    } else {
        $data->descriptionformat = $data->description_editor['format'];
        $data->description = $description = $data->description_editor['text'];
        $data->id = aceesscohort_add_cohort($data);
        $editoroptions['context'] = $context = context::instance_by_id($data->contextid);
        $data = file_postupdate_standard_editor($data, 'description', $editoroptions,
                $context, 'cohort', 'description', $data->id);
        if ($description != $data->description) {
            ...
            aceesscohort_update_cohort($updatedata);
        }
    }
    ...
    if ($returnurl->get_param('showall') || $returnurl->get_param('contextid') == $data->contextid) {
        ...
        redirect(new moodle_url('/local/accesscohort/index.php', array('contextid' => $data->contextid)));
    }
}
...
if (!$id && ($editcontrols = aceesscohort_edit_controls($context, $baseurl))) {
