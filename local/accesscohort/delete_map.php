...
if($createmapcap){
	...
	$PAGE->set_url('/local/accesscohort/delete_map.php');
	$delete = $DB->delete_records('local_mapping_cohort', array('id' => $id));
	if($delete) {
		redirect(new moodle_url($CFG->wwwroot.'/local/accesscohort/list_mapping.php'),'Delete');
	} else {
		echo "error";
	}
}else{
