...
if ($confirm and confirm_sesskey()) {
    ...
    foreach ($rs as $user) {
        ...
        $unsuspendcheck = do_unsuspend_user($user); 
    ...
    if($unsuspendcheck) {
    $adminsentcheck = informed_admin_about_unsuspended_user($allsentuser);
    }
    $rs->close();
    ...
    if ($status) { //TODO--need to give the notification properly
        ...
        echo $OUTPUT->notify_problem('Error:.user is in unsuspended state.No need to unsuspend');
    }
    ...
    $formcontinue = new single_button(new moodle_url('user_unsuspend.php', array('confirm' => 1)), get_string('yes'));
    $formcancel = new single_button(new moodle_url('user_bulk.php'), get_string('no'), 'get');
    
    echo $OUTPUT->confirm(get_string('unsuspenduser', 'local_usermanagement', $usernames), $formcontinue, $formcancel);
}
