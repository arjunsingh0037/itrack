...
if(user_has_role_assignment($USER->id, 9, SYSCONTEXTID) || user_has_role_assignment($USER->id, 10, SYSCONTEXTID) || is_siteadmin()){
...
if ($usernew = $userform->get_data()) {
        ...
        if($usernew->idnumber == 1){            //account manager created by Superadmin
           ...
           //role_assign(9, $usernew->id, 1);
            if($usernew->role == 'partner'){
                ...
                role_assign(13, $usernew->id, 1);
                add_mentors($usernew); 
            }
        }elseif($usernew->idnumber == 10){
           role_assign(11, $usernew->id, 1); 
    ...
    if ($user->id == $USER->id) {
        ...
        }elseif(user_has_role_assignment($USER->id, 10, SYSCONTEXTID) || is_siteadmin()){
