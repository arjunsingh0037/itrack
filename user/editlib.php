...
function useredit_shared_definition(&$mform, $editoroptions, $filemanageroptions, $user) {
    ...
    foreach (useredit_get_required_name_fields() as $fullname) {
        ...
        if($fullname == 'lastname'){
            $mform->addElement('hidden', 'lastname',  'Name',  'maxlength="100" size="20"');
            if ($stringman->string_exists('missing lastname', 'core')) {
                $strmissingfield = get_string('missing lastname', 'core');
            } else {
                $strmissingfield = $strrequired;
            }
            $mform->addRule('lastname', $strmissingfield, 'required', null, 'client');
            $mform->setType('lastname', PARAM_NOTAGS);
            $mform->setDefault('lastname',$firstname);
        }else{
            $mform->addElement('text', $fullname,  'Name',  'maxlength="100" size="15"');
    ...
    foreach ($enabledusernamefields as $addname) {
        $mform->addElement('text', $addname,  get_string($addname), 'maxlength="100" size="15"');
    ...
    if ($user->id > 0 and !empty($CFG->emailchangeconfirmation) and !empty($user->preference_newemail)) {
        ...
        $mform->addElement('text', 'email', get_string('email'), 'maxlength="100" size="15"');
    ...
    // $mform->addElement('static', 'description','','Allocate Access Permissions for the Partner ');
    // $availablefromgroup=array();
    // $availablefromgroup[] =& $mform->createElement('date_selector', 'availablefrom', '');
    // $availablefromgroup[] =& $mform->createElement('checkbox', 'selectall', '', 'Select All');
