...
$PAGE->requires->js('/blocks/quickmailsms/validation.js');
...
class email_form extends moodleform {
    ...
    public function definition() {
        ...
        $group_options = empty($this->_customdata['groups']) ? array() : array(
            'allusers' => quickmailsms::_s('allusers')
        );

        $group_options['all'] = quickmailsms::_s('all_sections');
        ...
        if (!empty($CFG->block_quickmailsms_addionalemail)) {
        $mform->addElement('text', 'additional_emails', quickmailsms::_s('additional_emails'), array('style'=>'width: 50%;'));
        $mform->setType('additional_emails', PARAM_TEXT);
        $mform->addRule('additional_emails', 'One or more email addresses is invalid', 'callback', 'block_quickmailsms_mycallback', 'client');
        $mform->addHelpButton('additional_emails', 'additional_emails', 'block_quickmailsms');
        }
        $mform->addElement(
