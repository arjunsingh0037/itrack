...
abstract class quickmailsms {
     ...
     */
    const PAGE_TYPE = 'block-quickmailsms';
    ...
    static function process_attachments($context, $email, $table, $id) {
        ...
        $tree = $fs->get_area_tree(
            $context->id, 'block_quickmailsms',
            'attachment_' . $table, $id, 'id'
        );

        $base_url = "/$context->id/block_quickmailsms/attachment_{$table}/$id";
    ...
    static function load_config($courseid) {
        ...
        if (empty($config)) {
            ...
            $ferpa = get_config($m, 'block_quickmailsms_ferpa');
            ...
            $config = array(
             ...
             if ($allowstudents == -1) {
                 ...
                 $config['ferpa'] = get_config('moodle', 'block_quickmailsms_ferpa');    
        }
    ...
    static function list_entries($courseid, $type, $page, $perpage, $userid, $count, $can_delete) {
        ...
        if($courseid == '1'){
            $table->head= array(get_string('date'), quickmailsms::_s('subject'),get_string('action'), quickmailsms::_s('status'), quickmailsms::_s('failed_to_send_to'),quickmailsms::_s('send_again'));
        }
        else{
            $table->head= array(get_string('date'), quickmailsms::_s('subject'),
                quickmailsms::_s('attachment'), get_string('action'), quickmailsms::_s('status'), quickmailsms::_s('failed_to_send_to'),quickmailsms::_s('send_again'));        
        }
        ...
        foreach ($logs as $log) {
            ...
            if($courseid == '1'){
                ...
                new moodle_url('/blocks/quickmailsms/admin_email.php', $params),
            ...
            $statusSENTorNot = quickmailsms::_s($type."success");
            
            if ( ! empty ($array_of_failed_user_ids) ){
                $statusSENTorNot = quickmailsms::_s('message_failure');
                $params += array(
                    'fmid' => 1,
                );
                $text = quickmailsms::_s('send_again');            
                
                if($courseid == '1'){
                    $sendagain = html_writer::link(new moodle_url("/blocks/quickmailsms/admin_email.php", $params), $text);
                }else{
                    $sendagain = html_writer::link(new moodle_url("/blocks/quickmailsms/email.php", $params), $text);
                }
                $listFailIDs = count($array_of_failed_user_ids);
                
                $failCount =  (($listFailIDs === 1) ?  $listFailIDs . " " . quickmailsms::_s("user") :  $listFailIDs . " " . quickmailsms::_s("users"));         
        ...
        $paging = $OUTPUT->paging_bar($count, $page, $perpage,
            '/blocks/quickmailsms/emaillog.php?type='.$type.'&amp;courseid='.$courseid.'&userid='.$userid);
...
function block_quickmailsms_pluginfile($course, $record, $context, $filearea, $args, $forcedownload) {
    ...
    if (!empty($CFG->block_quickmailsms_downloads)) {
    ...
    if ($filearea == 'attachment_log') {
        $time = $DB->get_field('block_quickmailsms_log', 'time', array(
            'id' => $itemid
        ));

        if ("{$time}_attachments.zip" == $filename) {
            $path = quickmailsms::zip_attachments($context, 'log', $itemid);
