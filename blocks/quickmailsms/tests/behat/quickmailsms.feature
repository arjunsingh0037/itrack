@block @block_quickmailsms @WIP @javascript
Feature: Verify course-level behavior of QuickmailSMS
  In order to communicate with members of my course
    As an instructor
    I need the ability to use and configure QuickmailSMS on a per-course basis.
...
Scenario: Test that form re-populates correctly.
    Given I click on "Compose New Email" "link" in the "QuickmailSMS" "block"
...
Scenario: As an Instructor, configure QuickmailSMS to allow/disallow student use.
    Given I log out
    ...
    When  I set the field "Allow students to use QuickmailSMS" to "Yes"
...
Scenario: Setup signature.
    Given I click on "Signatures" "link" in the "QuickmailSMS" "block"
    And I set the following fields to these values:
        | Title     | Test Sig  |
