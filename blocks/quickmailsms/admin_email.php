...
$blockname  = get_string('pluginname', 'block_quickmailsms');
$header     = get_string('sendadmin', 'block_quickmailsms');

$context    = context_system::instance();

if(has_capability('block/quickmailsms:myaddinstance', $context) || is_siteadmin($USER)) {
    // page params for ui filter
    $filterparams = $typeid > 0 ? array('courseid'=>$courseid, 'type'=>$type, 'typeid'=>$typeid) : null;

    $PAGE->set_context($context);
    $PAGE->set_url($CFG->wwwroot . '/blocks/quickmailsms/admin_email.php');
    $PAGE->navbar->add($blockname);
    $PAGE->navbar->add($header);
    $PAGE->set_heading($SITE->shortname.': '.$blockname);

    if($type == 'log'){
        $log_message = $DB->get_record('block_quickmailsms_' . $type, array('id' => $typeid));
    ...
    if ($form->is_cancelled()) {
        ...
        redirect(new moodle_url('/blocks/quickmailsms/admin_email.php'));
        ...
        if(empty($message->warnings)) {
            ...
            $data->status = quickmailsms::_s('failed_to_send_to') + count($message->warnings) + quickmailsms::_s('users');
    ...
    if(!empty($type)) {
        ...
        $log_message = file_prepare_standard_editor(
            $log_message, 'message', $editor_options, $context, 'block_quickmailsms', $type, $log_message->id
        );
    }else{
    ...
    $paging_bar = !$sql ? '' :
        $OUTPUT->paging_bar($usersearchcount, $page, $perpage,
            new moodle_url('/blocks/quickmailsms/admin_email.php', array(
                'sort' => $sort,
                'dir' => $direction,
                'perpage' => $perpage
            )
