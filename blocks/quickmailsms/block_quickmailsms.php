...
class block_quickmailsms extends block_list {
    ...
    function applicable_formats() {
        global $USER;
        if(is_siteadmin($USER->id) || has_capability('block/quickmailsms:myaddinstance', context_system::instance())) {
...
/*
            ...
            */
            ...
            $config = html_writer::link(
                new moodle_url('/blocks/quickmailsms/config_qm.php', $cparam),
                $icon.$config_str
            );
        ...
        if((has_capability('block/quickmailsms:myaddinstance', context_system::instance()) || is_siteadmin($USER->id)) && $COURSE->id == SITEID) {
            $send_adminemail_str = quickmailsms::_s('sendadmin');
